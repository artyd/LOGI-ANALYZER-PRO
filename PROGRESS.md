...
- [x] **Точність аналізу (6 ітерацій):** токен-матчер з упевненістю + стемінг (відмінки/солеві форми) `lib/engines/match.ts`; оверлей перевірених кодів `uktzed_code_db_extra.ts` (амінокислоти/вітаміни/АФІ); пінінг походження з бази + fallback за виробником; перевірки за категорією `categoryChecks`; грунтування AI впевненістю; режим ПДВ 20/7/0. **Межа евристик — далі потрібен офіційний тариф UA (Neon DATABASE_URL).**
- [ ] Реальний Митний тариф UA + валідація кодів (потребує DATABASE_URL від власника).
