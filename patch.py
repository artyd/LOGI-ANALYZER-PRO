...
def patch():
    ...
    async function analyzeBatch(batch, apiKey, euRegulations, batchIndex, totalBatches) {
      const systemPrompt = `Ти — професійний аналітик міжнародної логістики.
...
1. Розрахуй митні платежі (dutyRatePercent, напр. 6.5 для 6.5%). Якщо ціни/кількості немає, 0.
...
ФОРМАТ СУВОРО JSON:
{
  ...
  "criticalAlert": "небезпека словами або null",
...
Уважно перевір щоб в items було рівно ${batch.length} об'єктів. Відповідь виключно в JSON!`;
    ...
    content = content.replace("  <script>\n    let apiKey = localStorage.getItem", js_code)

    # 2. Replace runAudit body
    block_start = "        let allSheetsData = '';"
    block_end = "        uploadedFileData = null;\n        document.getElementById('fileName').style.display = 'none';\n        document.getElementById('fileUpload').value = '';\n\n      } catch (e) {"
    ...
    if idx_start != -1 and idx_end != -1:
        ...
        after = content[idx_end + len(block_end):]
    ...
    ft_start = "function formatTableForAI(rows) {"
    ft_end_string = "return output;\n    }"
    
    idx_ft = content.find(ft_start)
    if idx_ft != -1:
        idx_ft_end = content.find(ft_end_string, idx_ft)
        if idx_ft_end != -1:
            before_ft = content[:idx_ft]
            after_ft = content[idx_ft_end + len(ft_end_string):]
            content = before_ft + after_ft

    with open('index.html', 'w', encoding='utf-8', errors='surrogateescape') as f:
        f.write(content)
    print("PATCH_SUCCESS surrogateescape")
